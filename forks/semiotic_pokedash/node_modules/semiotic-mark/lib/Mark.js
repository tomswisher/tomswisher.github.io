    ...
    value: function shouldComponentUpdate(nextProps) {
      ...
      _markTransition.attributeTransitionWhitelist.forEach(function (attr) {
        if ((0, _d3Selection.select)(node).select("*").transition) {
          if (attr === "d" && (0, _markTransition.differentD)(cloneProps.d, this.props.d)) {
            (0, _d3Selection.select)(node).select("*").attr("d", cloneProps.d);
            ...
            (0, _d3Selection.select)(node).select("*").transition(attr).duration(appliedDuration).attr(attr, cloneProps[attr]);
          }
        }
      }, this);

      if (cloneProps.style) {
        _markTransition.attributeTransitionWhitelist.forEach(function (style) {
          if (cloneProps.style[style] !== this.props.style[style]) {
            ...
            if ((0, _d3Selection.select)(node).select("*").transition) {
              ...
              (0, _d3Selection.select)(node).select("*").transition(style).duration(appliedDuration).style(style, nextValue);
            } else {
